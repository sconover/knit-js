knit.engine.Memory.Relation = function() {
  ...
  p.objects = function(rows) {
    ...
    return _.map(rows, function(row){
      var object = {}
      _.each(self.attributes(), function(attr, columnPosition) {
  ...
  p.performUnnest = function(nestedAttribute) {
    var newAttributes = this.attributes().shallowCopy()
    var nestedAttributeIndex = knit.indexOfSame(newAttributes, nestedAttribute)
    newAttributes.splice.apply(newAttributes, [nestedAttributeIndex,1].concat(nestedAttribute.nestedRelation().attributes().toArray()))
