  ...
  knit.ReferenceEnvironment = function(){
    ...
    p.resolve = function(externalBindings) {
      ...
      function resolveRelation(resolved, relationKey) {
        _A.each(_.keys(externalBindings), function(relationKey){

          self.relation(relationKey).resolve(externalBindings)
          resolved.push(relationKey)

          _A.each(externalBindings[relationKey].attributes(), function(attribute){
      ...
      var resolved = []
      _A.each(_.keys(externalBindings), function(relationKey){ resolveRelation(resolved, relationKey) })
    
      var stillToResolve = _A.differ(_.keys(this._keyToRef), resolved)
      var allBindings = _.extend(externalBindings, this._internalBindings)
      _A.each(stillToResolve, function(key){ self._keyToRef[key].resolve(allBindings) })
    ...
    p.decorate = function(target, bindings) {
      ...
      var self = this
      var originalRename = target.rename
