knit.translation.sql = function(){
  ...
  var Select = function() {
      ...
      p.attributes = function(){ 
        ...
        return new knit.Attributes(_.map(this._whats, function(sqlColumn){
          var parts = sqlColumn.name.split("."),
              ...
              table = _.detect(self._froms, function(table){return table.name() == tableName})
    ...
    p._resetWhats = function() {
      var self = this
      function allColumnsFromFroms() {
        var allColumns = new knit.Attributes([])
        _.each(self._froms, function(from) {
          allColumns = allColumns.concat(from.attributes())
        })
        return allColumns
      }
      
      if (_.empty(this._suppliedWhats)) {
        this._whats = allColumnsFromFroms().map(function(col){return Column.fromAttribute(col)})
      } else {
    ...
    _.each(["join", "where", "order"], function(appenderMethodName){
