...
;(function(){
  ...
  sql.Select.prototype.toStatement = function() {
    ...
    function allTables() {
      return _.map(self._froms, function(from){
        var table = _.values(from)[0]
        return table
      })
    }
    function allAttributes() {
      var allAttributes = new knit.Attributes([])
      _.each(allTables(), function(table){
        allAttributes = allAttributes.concat(table.attributes())
      })
      return allAttributes
    }
    function selectClause() {
      var allColumnsFromAllTables = allAttributes().map(function(attr){return sql.Column.fromAttribute(attr)})
      var whats = _.deepSame(allColumnsFromAllTables, self._whats) ?
                    [new sql.Wildcard()] :
                    self._whats
      return _.map(whats, function(what){return what.toString()}).join(", ")
    }
    function from() { 
      return " from " + 
              _.map(self._froms, function(from){
                var table = _.values(from)[0]
                return table.name()
              }).join(", ") }
