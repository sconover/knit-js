...
knit.algebra.Attributes = function() {
  ...
  p.spliceInNestedAttribute = function(nestedAttribute) {
    var self = this
    var attributePositions = _.map(nestedAttribute.nestedRelation().attributes(), function(attr){
      return _.indexOf(self, attr)
    })
    
    attributePositions.sort()
    var firstNestedAttributePosition = attributePositions[0]
    
    var newAttributes = _.differ(this, nestedAttribute.nestedRelation().attributes())
    return _.splice(newAttributes, new F([nestedAttribute]), firstNestedAttributePosition)
  }
