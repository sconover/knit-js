knit.engine.Memory.Relation = function() {
  ...
  p.inspect = function() {
    return this.name() + "[" + this.attributes().inspect() + "]"
  }
  ...
  p.objects = function(rows) {
    ...
    return _.map(rows, function(row){
      return self.attributes().makeObjectFromRow(row)
    })
  }
  ...
  function rowWithAttributes(row, attributes) {
    ...
    _.each(attributes, function(attr, i){
      rowWithAttributes.push([attr, row[i]])
    })
    ...
    return this._newRelation(
      joinRows(
        ...
        function(rightAttributes, leftRow, joinRows){
          var rightAsNulls = []
          _.times(rightAttributes.size(), function(){rightAsNulls.push(null)})
    ...
    return this._newRelation(
      joinRows(
        ...
        function(leftAttributes, rightRow, joinRows){
          var leftAsNulls = []
          _.times(leftAttributes.size(), function(){leftAsNulls.push(null)})
  ...
  p.performUnnest = function(nestedAttribute) {
    ...
    var newAttributes = this.attributes().splice2(nestedAttribute.nestedRelation().attributes(), nestedAttributeIndex, 1)

    var unnestedRows = []

    knit._.each(this.rows(), function(row) {
      var nestedRows = row[nestedAttributeIndex]
      knit._.each(nestedRows, function(nestedRow){
        unnestedRows.push(knit._.splice(row, nestedRow, nestedAttributeIndex, 1))
      })
    })
