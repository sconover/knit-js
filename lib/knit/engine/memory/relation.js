knit.engine.Memory.Relation = function() {
  ...
  p.performJoin = function(relationTwo, predicate, noRightMatchFound) {
    noRightMatchFound = noRightMatchFound || function(){}
    var leftRows = this.rows()
    var rightRows = relationTwo.rows()
    var combinedAttributes = [].concat(this.attributes()).concat(relationTwo.attributes())
    var joinRows = []
    var self = this
    
    _.each(leftRows, function(leftRow){
      var rightRowMatchFound = false
      _.each(rightRows, function(rightRow){
        var candidateJoinRow = [].concat(leftRow).concat(rightRow)
        if (predicate.match(self._rowWithAttributes(candidateJoinRow, combinedAttributes))) {
          joinRows.push(candidateJoinRow)
          rightRowMatchFound = true
        }
      })
      
      if ( ! rightRowMatchFound) {
        noRightMatchFound(relationTwo, leftRow, joinRows)
      }
    })
  ...
  p.performLeftOuterJoin = function(relationTwo, predicate) {
    return this.performJoin(relationTwo, predicate, function(relationTwo, leftRow, joinRows){
      var rightAsNulls = []
      _.times(relationTwo.attributes().length, function(){rightAsNulls.push(null)})
