knit.engine.memory.Relation = function() {
  ...
  p.performSelect = function(predicate) {
    var attributes = this.attributes()
        rawPredicate = function(attributeNames, row) {
    ...
    return this._newRelation(result.rows, this.name(), attributes.get(result.attributes)) 
  }
  
  p.performProject = function(keepAttributes) {
    var result = knit.algorithms.project(this, keepAttributes)
    return this._newRelation(result.rows, this.name(), result.attributes) 
  }
    ...
    return this._newRelation(
      joinRows(
        ...
        function(rightAttributes, leftRow, joinRows){
          var rightAsNulls = _.repeat([null], rightAttributes.size()),
