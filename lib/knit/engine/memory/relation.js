knit.engine.memory.Relation = function() {
  ...
  p.performSelect = function(predicate) {
    var attributes = this.attributes(),
        ...
        result = knit.algorithms.select({attributes:attributes.names(), rows:this.rows()}, rawPredicate)
  ...
  p.performDivide = function(divisor) {
    var quotientName = this.name() + "$$" + divisor.name()
    
    var result = knit.algorithms.divide(
      {attributes:this.attributes().fullyQualifiedNames(), rows:this.rows()},
      {attributes:divisor.attributes().fullyQualifiedNames(), rows:divisor.rows()}
    )

    return this._newRelation(result.rows, quotientName, this.attributes().differ(divisor.attributes()))
  }
  ...
  p.performUnnest = function(nestedAttribute) {
    ...
    var allAttributes = this.attributes().concat(nestedAttribute.nestedRelation().attributes())
    return this._newRelation(result.rows, this.name(), allAttributes.fromPrimitives(result.attributes))
  }
